...
class SimulationService extends NFNService {
  ...
  override def function(interestName: CCNName, argSeq: Seq[NFNValue], ccnApi: ActorRef): NFNValue = {
    // /path/to/NBodySimulation /path/to/config '-d' <deltaTime> '-s' <stepCount> '-i' <intermediateInterval>

    var options = Map('deltaTime -> 60, 'stepCount -> 1000, 'intermediateInterval -> 0)
