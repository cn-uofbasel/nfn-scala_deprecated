 ...
 * The service names (with prefix) are directly adjusted to the NDNExSetup!
 *
 * Terminology:
...
object InterestBuilderStatic {
  ...
  val permissionChannelDirectName: CCNName = CCNName("serviceprovider/health/filtering/filterAccess_service_permission_PermissionChannelStorage")
  ...
  val keyChannelDirectName: CCNName = CCNName("serviceprovider/health/filtering/filterAccess_service_key_KeyChannelStorage")
  ...
  /** Hard-coded ccn name of service for content channel (filtering) (direct) */
  val contentChannelFilteringDirectName: CCNName = CCNName("serviceprovider/health/filtering/filterAccess_service_content_ContentChannelFiltering")

  /** Hard-coded ccn name of service for content channel (filtering) (indirect) */
  val contentChannelFilteringIndirectName: CCNName = CCNName("own/machine/filterAccess_service_content_ProxyContentChannel")
  ...
  def buildDirectContentChannelInterest(name: String, accessLevel: Int): Interest = {
    ...
    accessLevel match {
      ...
      case l if l > 1 => contentChannelFilteringDirectName call(name, l)
    }
  ...
  def buildIndirectContentChannelInterest(name: String, accessLevel: Int): Interest = {
    ...
    accessLevel match {
      ...
      case l if l > 1 => contentChannelFilteringIndirectName call(name, l)
    }
